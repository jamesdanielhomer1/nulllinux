...
PLYMOUTH_SPLASH=
# The SHIPPED theme, not system/plymouth-theme -- that path is gitignored and
# generated by null-build, so it exists only on a build machine; the package
# ships the theme under assets/prebuilt/boot (the prebuilt tarball). Using the
# build-only path baked NO splash on a real install: the check silently fell to
# "theme missing".
_theme=/opt/nulllinux/assets/prebuilt/boot/plymouth-theme
